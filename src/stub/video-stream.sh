...
ffmpeg -s $video_width\x$video_height -f video4linux2 -i $usb_camera -r $video_frame \
    ...
    -pix_fmt yuv420p -an -c:v h264 -b:v 2048k -profile:v baseline -tune zerolatency -flags global_header -bsf dump_extra \
    -f rtp "rtp://$video_server:$video_port/?pkt_size=1000" -payload_type 96 -fflags nobuffer -strict experimental -flush_packets 1
